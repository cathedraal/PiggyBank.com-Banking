...
import {
  renderSuccessPage,
  renderFailPage,
} from '../interfaces/validationPages.js';
...
export function renderCurrency(section, text, boolean, existingCurrency) {
  ...
  for (let i = 0; i < currencies.length; i++) {
    ...
    menuCurrencyButton.addEventListener('click', function () {
      ...
      const allCurrencyButtons = document.querySelectorAll(
        '.menu-currencyButton'
      );
...
export function renderWallet(section) {
  ...
  menuWalletCardNumber.textContent = maskCardNumber(
    user.userCards[0].cardNumber
  );
  ...
  menuManageWalletTransferIconDiv.classList.add(
  ...
  menuManageWalletWithdrawIconDiv.classList.add(
...
export function renderChooseWallet(section, context) {
  ...
  for (let i = 0; i < user.userCards.length; i++) {
    ...
    addMoneyWalletNumber.textContent = maskCardNumber(
      user.userCards[i].cardNumber
    );
    ...
    addMoneyWallet.addEventListener('click', function () {
      ...
      allMoneyWallets.forEach((button) =>
        button.classList.remove(
          'addMoney-wallet--active',
          (button.style.backgroundColor = '#f8f8f8')
        )
      );
    ...
    if (inputPressed && amountOfMoney !== 0) {
      if (context === 'Add money') {
        ...
        const action = new Action(
          'images/actions_add.png',
          'Added money',
          `${selectedCard.cardCurrency}${amountOfMoney}`,
          dateTime,
          'Add',
          source
        );
        ...
        const success =
          amountOfMoney < selectedCard.cardBalance &&
          recipientname !== '' &&
          recipientemail !== '';
        if (success) {
          selectedCard.transferMoney(
            recipientname,
            recipientemail,
            recipienttext,
            user.userName,
          ...
          const action = new Action(
            'images/actions_transfer.png',
            'Sent money to',
            `${recipientname} ${selectedCard.cardCurrency}${amountOfMoney}`,
            dateTime,
            'Transfer',
            source
          );
